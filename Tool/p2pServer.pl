#!/usr/bin/perl
#================================================================--
# File Name    : p2pServer.pl
#
# Purpose      : Implement the Virtual p2p infrastructure for csci 460
#
...
sub prompt {
   push(@stdout_fifo, "p2pServer[$portsOpen]> ");
...
my $mcb = Exc::TryCatch->new(
   fn => sub {
      for (my $i=0; $i<$maxPorts; $i++) {
         ...
         if (defined($msg)) {
            my $startFrom=int($i / 2) * 2; # 2 ports per p2p
            for (my $j=$startFrom; $j<$startFrom+2; $j++) {
